...
transformed parameters {
  ...
  for (t in (max_Lag+1):N_time) {
    alpha_MarSun[t] = mu[t] + b_temp * temp_MarSun[t-Lag_temp] + b_light * light_MarSun[t-Lag_light] + b_SIG5 * SIG5_MarSun[t-Lag_SIG5];
  }
  for (t in (max_Lag+1):N_time) {
    alpha_MarShade[t] = mu[t] + b_temp * temp_MarShade[t-Lag_temp] + b_light * light_MarShade[t-Lag_light] + b_SIG5 * SIG5_MarShade[t-Lag_SIG5];
  }
  for (t in (max_Lag+1):N_time) {
    alpha_SepSun[t] = mu[t] + b_temp * temp_SepSun[t-Lag_temp] + b_light * light_SepSun[t-Lag_light] + b_SIG5 * SIG5_SepSun[t-Lag_SIG5];
  }
  for (t in (max_Lag+1):N_time) {
    alpha_SepShade[t] = mu[t] + b_temp * temp_SepShade[t-Lag_temp] + b_light * light_SepShade[t-Lag_light] + b_SIG5 * SIG5_SepShade[t-Lag_SIG5];
  }
}
