...
Lag_env <- expand.grid(list(
  Lag_temp=c(0),
  Lag_light=c(0),
  Lag_CCA1=c(0, 1, 2, 3, 4, 5)))
  

## Load stan model
model <- cmdstan_model("stan_model/LLMX_MarSep_SIG5.stan")
...
for(i in 1){
  ...
  max_Lag <- max(c(Lag_env$Lag_temp[i], Lag_env$Lag_light[i], Lag_env$Lag_CCA1[i]))
  ...
  data_list <- list(
    ...
    Y_MarSun = data_MarSun$SIG5,
    Y_MarShade = data_MarShade$SIG5,
    Y_SepSun = data_SepSun$SIG5,
    Y_SepShade = data_SepShade$SIG5
    )
  
  out_name <- paste0("LLMX_MarSep_SIG5_", "Lagtemp", data_list$Lag_temp, "_Laglight", data_list$Lag_light, "_LagCCA1", data_list$Lag_CCA1)
  ps <- 7
  
  # SSM model
  fit <- model$sample(
    data = data_list,
    init = function() { list(alpha_MarSun = as.numeric(tapply(data_MarSun$SIG5, data_MarSun$Time, mean, na.rm=T)),
                             alpha_MarShade = as.numeric(tapply(data_MarShade$SIG5, data_MarShade$Time, mean, na.rm=T)),
                             alpha_SepSun = as.numeric(tapply(data_SepSun$SIG5, data_SepSun$Time, mean, na.rm=T)),
                             alpha_SepShade = as.numeric(tapply(data_SepShade$SIG5, data_SepShade$Time, mean, na.rm=T))) },
    seed = 10,
    iter_warmup = 3000,
    iter_sampling = 1000,
    #thin = 3,
    ...
    sig_figs = 4,
    output_dir = "/tmp",
    output_basename = paste0("SIG5_LLMX", i)
  )
  
  # 99% Bayesian credible intervals
  outcsv_name <- list.files("/tmp")
  outcsv_name <- outcsv_name[grep(paste0("SIG5_LLMX", i), outcsv_name)]
  tmp_csv_mu <- NULL
  tmp_csv_b_temp <- NULL
  ...
  for(j in 1:length(outcsv_name)){
    ...
    tmp_csv_b_CCA1 <- c(tmp_csv_b_CCA1, tmp_csv[,str_starts(names(tmp_csv), "b_CCA1")])
  ...
  df_b_CCA1 <- as.data.frame(round(t(quantile99(tmp_csv_b_CCA1)), digits = 4))
  row.names(df_b_CCA1) <- "b_CCA1"
  ...
  df_all <- rbind(df_mu, df_b_temp, df_b_light, df_b_CCA1, df_s_mu, df_s_Y, df_alpha_MarSun, df_alpha_MarShade, df_alpha_SepSun, df_alpha_SepShade)
  df_all <- df_all %>%
  ...
  # MCMC samples
  tmp_csv_all <- cbind(tmp_csv_mu, tmp_csv_b_temp, tmp_csv_b_light, tmp_csv_b_CCA1, tmp_csv_s_mu, tmp_csv_s_Y, tmp_csv_alpha_MarSun, tmp_csv_alpha_MarShade, tmp_csv_alpha_SepSun, tmp_csv_alpha_SepShade)
  ...
  g <- bayesplot::mcmc_combo(
    ...
    widths = c(1, 1),
    pars = c(paste0("mu[", data_list$max_Lag+1,"]"), paste0("mu[", data_list$N_time, "]"), "b_temp", "b_light", "b_CCA1"),
